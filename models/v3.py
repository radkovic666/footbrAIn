...
features = [
    ...
    'relative_strength', 'power_rank_diff', 'win_rate_diff', 'away_conceded','home_conceded','trend_diff'
]
...
joblib.dump(stacking_clf, "match_outcome_model_v3.pkl")
print("✅ Model saved to match_outcome_model_v3.pkl")
...
plt.title("v3 Feature Importances")
plt.bar(range(len(importances)), importances[indices], align="center")
plt.xticks(range(len(importances)), [feature_names[i] for i in indices], rotation=45)
plt.tight_layout()
plt.savefig("v3_feature_importance.png")
plt.close()

# Generate permutation importance plot
plt.figure(figsize=(12, 8))
plt.title("v3 Permutation Importances (Mean ± Std Dev)", fontsize=14, pad=20)
...
# Save and close
plt.savefig("v3_permutation_importance.png", dpi=300, bbox_inches='tight')
plt.close()
...
plt.savefig("v3_shap_summary.png")
plt.close()
